...
public class Calculator extends VBox implements EventHandler<ActionEvent> {
	...
	public void handle(ActionEvent event) {
		ArrayList<String> numsList = new ArrayList<>(Arrays.asList("1", "2", "3", "4", "5", "6", "7", "8", "9", "0"));
		...
		if (numsList.contains(value)) {
			...
			if (!number1.equals("") && !number2.equals("") && operator != null) {
				double result = 0;
				...
				switch (operator) {
					...
					case "/" -> {
						if (num2 != 0)
							result = num1 / num2;
						else {displayText.setText("¡Peligro! vas a dañar el universo");
						return;}
					}
				}
	...
	private boolean isOperador(String value) {
		...
		return operadores.contains(value);
	}
}